...
{
    [TestClass]
    public class Prerequisiteold : TestCase
    {
        ...
        public void Prerequisites()
        {
            ...
            try
            {
                ...
                for (int count1 = 1; count1 <= 2; count1++)
                {
                    ...
                    if (null == userConfigurationPage)
                    {
                        ...
                        stepNo++;
                        userConfigurationPage = userDetailsPage.AddUser(user, emailAddress, userPassword);
                ...
                for (applicationNumber = 1; applicationNumber <= count; applicationNumber++)
                {
                    ...
                    if (null == applicationPage)
                    {
                        ...
                        {
                            editApplicationPage = addApplicationPage.addApplicationDetails(applicationName, applicationName, false, false, false, false, false, true, null);
                        ...
                        {
                            editApplicationPage = addApplicationPage.addApplicationDetails(applicationName, applicationName, false, false, false, false, false, false, null);
                            Assert.IsNotNull(editApplicationPage, "Application Added Failed");
                            WriteLogs(testScriptName, stepNo, "Application Added Succesfully ", "PASS", browser);
                        }
                        else
                        {
                            editApplicationPage = addApplicationPage.addApplicationDetails(applicationName, applicationName, true, false, false, false, false, false, null);
                            Assert.IsNotNull(editApplicationPage, "Application Added Failed");
                            WriteLogs(testScriptName, stepNo, "Application Added Succesfully ", "PASS", browser);
                        }
            ...
            {
                stepNo++;
                WriteLogs(testScriptName, stepNo, "Execution of script terminated. Exception is " + exception.ToString(), "FAIL", browser);
